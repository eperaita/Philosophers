...
/*   Updated: 2021/12/09 12:16:24 by eperaita         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int table_dead(t_philo *philo)
{
...
{
	if(table_dead(philo))//hay alguien muerto!!!! KILL THE PROGRAM
        return(1);
	else if(is_dead(philo) && !table_dead(philo)) //estoy muerta y nadie mas
	{
		pthread_mutex_lock(&philo->table->deadlock);
		philo->table->death = 1;
		pthread_mutex_unlock(&philo->table->deadlock);
